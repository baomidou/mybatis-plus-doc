...
在版本 `v3.5.9` 之后的版本重构 `service` 模块抽象为 `repository` 建议使用 `IRepository` 避免语义层面混乱，使用上与接口 `IService` 用法一致。

遵循 `Controller（表现层） -> Service（业务逻辑层） -> Repository（逻辑数据访问层） -> Mapper（物理数据访问层）` 的分层架构，虽然初期会多一些代码量，
但它带来的职责清晰、易于维护、方便测试和扩展性强等好处，对于构建健壮的企业级应用至关重要。
...
:::tip
如果想 `架构更加简单一点` 追求简洁也可以合并 `Service（业务逻辑层） Repository（逻辑数据访问层）` 这两层统一为 `Service（业务逻辑数据处理层）` 需要注意的是无论何种架构分层 `Mapper（物理数据访问层）` 都不建议编写任何业务逻辑数据处理代码。
:::
...
- 在版本 `3.5.9` 之后的版本建议使用 `IRepository` 逻辑上没有区别，只是语义上更加清晰。
...
```java
// 设置查询条件为 age > 25，并将结果转换为 String 列表
List<String> userNames = userService.listObjs(Wrappers.<SysUser>lambdaQuery().select(SysUser::getName)
.gt(SysUser::getAge, 25), obj -> (String) obj);
