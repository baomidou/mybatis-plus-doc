...
因为 MP 底层是 MyBatis，所以你要知道 MP 只是帮你注入了常用 CRUD 到 MyBatis 里，注入之前可以说是动态的（根据您的 Entity 字段以及注解变化而变化），但是注入之后是静态的（等于 XML 配置中的内容）。

---

而对于直接指定 `typeHandler` ，Mybatis 只支持写在 2 个地方:

1. 定义在 resultMap 里，作用于 Select 查询的返回结果封装
2. 定义在 `insert` 和 `update` SQL 语句 的 `#{property}` 里的 `property` 后面（例:`#{property,typehandler=xxx.xxx.xxx}`），并且只作用于 `设置值`

而除了这两种直接指定 `typeHandler` ，MyBatis 有一个全局的扫描您自己的 `typeHandler` 包的配置，这是根据您的 `property` 的类型去找 `typeHandler` 并使用.
...
| insertStrategy   | Enum                         | N        | DEFAULT                  | 举例：NOT_NULL <br/> `insert into table_a(<if test="columnProperty != null">column</if>) values (<if test="columnProperty != null">#{columnProperty}</if>)`                                                                                |
| updateStrategy   | Enum                         | N        | DEFAULT                  | 举例：IGNORED <br/> `update table_a set column=#{columnProperty}`                                                                                                                                                                          |
| whereStrategy    | Enum                         | N        | DEFAULT                  | 举例：NOT_EMPTY <br/> `where <if test="columnProperty != null and columnProperty!=''">column=#{columnProperty}</if>`                                                                                                                       |
